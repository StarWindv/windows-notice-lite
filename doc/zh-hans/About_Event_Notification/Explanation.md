...
其实吧, 我最开始真的是没想过做补救措施的, 那人家系统都这样了, 我能怎么办?

但是后来一想, 那 Toast 本身也不支持物理中断, 那也就是说属于它的事件回调仍然是交给操作系统来进行循环回调处理的

—— 那异步回调我自己也能写, tokio 的事件循环本来就足够的高效, 我努力的做调整, 我相信性能上不会差多少的,

脏活累活我全干了, 大家只需要用就好了, 这就是我最后所想的.

至于自适应间隔问题, 这个不是没想过, 但是比如说你server开机一个月没有发toast

然后自适应把查询间隔放到了一天一查

那万一在 31 天时来`error`了没获取到怎么办?

那还有人说把自适应上限放小一点, 那如果一个toast最多只能存活10秒, 把轮询上限放到一分钟, 错过消息了算谁的责任?
